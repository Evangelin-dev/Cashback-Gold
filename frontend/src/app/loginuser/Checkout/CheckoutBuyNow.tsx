...
const BuyNowCheckoutPage = () => {
  ...
  const { totalAmount, cartItems } = location.state || { cartItems: [] };
  ...
  useEffect(() => {
    if (!totalAmount || !singleItem) {
      navigate('/buyornaments');
    }
  }, [totalAmount, singleItem, navigate]);
  ...
  const handleProceedToPayment = async () => {
    ...
    if (!selectedAddress || !singleItem) {
      setSubmitError("Address or product details are missing.");
      return;
    }
    ...
    try {
      const buyNowPayload = {
        ornamentId: singleItem.ornament.id,
      ...
      const initiateRes = await axiosInstance.post('/api/orders/checkout/buy-now', buyNowPayload);
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          } catch (err: any) { setSubmitError(err.response?.data?.message || 'Payment verification failed.'); }
          finally { setIsSubmittingPayment(false); }
        },
        prefill: { name: selectedAddress.fullName, email: currentUser?.email || '', contact: selectedAddress.mobile },
        theme: { color: '#7a1335' },
    ...
    <>
      <div className="min-h-screen bg-slate-50 py-8">
        <div className="mx-auto max-w-5xl px-4">
          <h1 className="text-3xl font-bold text-center text-[#7a1436] sm:text-4xl mt-2 mb-8">Complete Your Purchase</h1>
            ...
            <div className="md:col-span-2">
              <div className="sticky top-32 rounded-2xl bg-white p-6 shadow-sm">
                ...
                <div className="flex items-center gap-4 border-b pb-4 mb-4">
                  <img src={singleItem.ornament.mainImage} alt={singleItem.ornament.name} className="h-20 w-20 rounded-lg object-cover bg-gray-100 flex-shrink-0" />
                  <div>
                    <p className="font-bold text-gray-800">{singleItem.ornament.name}</p>
                    <p className="text-sm text-gray-500">Quantity: {singleItem.quantity}</p>
                  </div>
                </div>
                <div className="flex justify-between text-xl font-bold text-gray-800"><span>To Pay</span><span>{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(totalAmount)}</span></div>
                {submitError && <div className="mt-4 text-center text-sm text-red-600 bg-red-50 p-2 rounded-lg">{submitError}</div>}
                <button onClick={handleProceedToPayment} disabled={!selectedAddressId || !isRazorpayLoaded || isSubmittingPayment} className="mt-6 w-full rounded-lg bg-[#7a1436] py-3 font-semibold text-white transition-all hover:scale-105 disabled:cursor-not-allowed disabled:bg-gray-400">{isSubmittingPayment ? <Loader2 className="mx-auto h-6 w-6 animate-spin" /> : !isRazorpayLoaded ? 'Loading Payment...' : `Pay Securely`}</button>
      ...
      {showSuccessPopup && (<Portal><div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"><div className="m-4 w-full max-w-sm rounded-2xl bg-white p-6 text-center shadow-xl"><h3 className="text-xl font-bold text-green-600">Order Placed!</h3><p className="mt-2 text-gray-600">Your order is confirmed. Thank you!</p><div className="mt-6 flex justify-center gap-4"><button onClick={() => navigate('/user')} className="rounded-lg bg-green-600 px-6 py-2.5 font-semibold text-white transition-colors hover:bg-green-700">View Orders</button></div></div></div></Portal>)}
    </>
  );
};
