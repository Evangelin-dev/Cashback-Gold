import { Award, Eye, Heart, Share2, Shield, ShoppingCart, Sparkles, Star, Zap, Loader } from 'lucide-react';
        ...
        if (mainProduct?.itemType) {
          try {
            const similarResponse = await axiosInstance.get(
              `/admin/ornaments/by-item-type?itemType=${mainProduct.itemType}&page=0&size=6`
            );
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 relative z-10">
          <div className="flex items-center justify-between py-3 mb-4">
            <div className="flex items-center space-x-1 text-xs text-[#7a1335]/70">
              <span className="hover:text-[#7a1335] cursor-pointer" onClick={() => navigate('/')}>Home</span>
              <span>/</span>
              <span className="hover:text-[#7a1335] cursor-pointer" onClick={() => navigate('/buyornaments')}>Jewelry</span>
              <span>/</span>
              ...
              <div className="relative group">
                <div className="aspect-square bg-gradient-to-br from-[#7a1335]/5 to-purple-100 rounded-2xl overflow-hidden shadow-2xl">
                  ...
                  <div className="absolute top-3 right-3 flex flex-col space-y-2">
                    <button onClick={() => setIsLiked(!isLiked)} className="p-2 bg-white/90 backdrop-blur-sm rounded-full shadow-lg hover:scale-110 transition-all duration-300"><Heart className={`w-4 h-4 ${isLiked ? 'text-red-500 fill-red-500' : 'text-[#7a1335]'}`} /></button>
                ...
                <div className="flex space-x-2 mt-3 overflow-x-auto pb-2">
                  {productImages.map((img, i) => (
                    <button key={i} onClick={() => setSelectedImage(i)} className={`flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden transition-all duration-300 ${selectedImage === i ? 'ring-2 ring-[#7a1335] scale-110 shadow-lg' : 'hover:scale-105 opacity-70 hover:opacity-100'}`}>
            ...
            <div className="lg:col-span-6 space-y-4 flex flex-col">
              <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-4 shadow-xl border border-white/20">
                ...
                <div className="flex items-center space-x-3">
                  <span className="text-3xl font-bold text-[#7a1335]">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(product.price)}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                {[ { icon: Award, label: "Material", value: product.material }, { icon: Shield, label: "Purity", value: product.purity }, { icon: Sparkles, label: "Quality", value: product.quality }, { icon: Zap, label: "Warranty", value: product.warranty }].map((item, index) => (
                  <div key={index} className="bg-white/80 backdrop-blur-sm rounded-xl p-3 shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300 group">
                    <div className="flex items-center space-x-2"><item.icon className="w-4 h-4 text-[#7a1335] group-hover:scale-110 transition-transform" /><div><div className="text-xs text-[#7a1335]/70">{item.label}</div><div className="font-semibold text-[#7a1335] text-sm">{item.value}</div></div></div>
                  </div>
                ))}
              </div>

              {/* --- FIX: REMOVED mt-auto TO PREVENT LARGE SPACE --- */}
              <div className="grid grid-cols-2 gap-4 pt-4">
                <button className="flex w-full items-center justify-center gap-3 rounded-2xl border-2 border-[#7a1335] bg-transparent py-4 px-6 font-bold text-[#7a1335] transition-all duration-300 ease-in-out hover:bg-[#7a1335] hover:text-black hover:shadow-lg hover:-translate-y-0.5"><ShoppingCart className="h-5 w-5" /><span>Add to Cart</span></button>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 mb-12">
            <div className="flex border-b border-[#7a1335]/20">
              {['details', 'pricing'].map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={`px-6 py-3 font-medium text-sm transition-all duration-300 ${activeTab === tab ? 'text-[#7a1335] border-b-2 border-[#7a1335] bg-[#7a1335]/5' : 'text-[#7a1335]/70 hover:text-[#7a1335] hover:bg-[#7a1335]/5'}`}>{tab.charAt(0).toUpperCase() + tab.slice(1)}</button>))}
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {similarProducts.map((p) => (
                  <div key={p.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group cursor-pointer" onClick={() => navigate(`/buyornaments/${p.id}`)}>
                    <div className="aspect-square overflow-hidden"><img src={p.mainImage} alt={p.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" /></div>
                    <div className="p-4">
                      <h4 className="font-semibold text-gray-800 mb-1 truncate">{p.name}</h4>
                      <p className="text-lg font-bold text-[#7a1335] mb-2">{new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(p.price)}</p>
