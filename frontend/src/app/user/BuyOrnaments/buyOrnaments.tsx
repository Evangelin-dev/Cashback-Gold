            ...
            <>
                    ...
                    <div style={{ padding: '0 32px 32px 32px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                      ...
                      <div style={{ marginTop: 'auto' }}>
                        ...
                        <div style={{ display: 'flex', gap: '12px' }}>
                          ...
                          <button 
                            style={{ width: '56px', height: '56px', backgroundColor: addedToCart.has(product.id) ? '#22c55e' : 'rgba(122, 19, 53, 0.1)', color: addedToCart.has(product.id) ? '#ffffff' : '#7a1335', border: '2px solid', borderColor: addedToCart.has(product.id) ? 'transparent' : 'rgba(122, 19, 53, 0.2)', borderRadius: '16px', cursor: 'pointer', transition: 'all 0.3s ease', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
              ...
              {hasMore && (
                <div style={{ marginTop: '48px', display: 'flex', justifyContent: 'center' }}>
                  <button onClick={() => fetchOrnaments(currentPage)} disabled={loadingMore} style={{ padding: '12px 24px', background: '#7a1335', color: 'white', border: 'none', borderRadius: '12px', cursor: loadingMore ? 'not-allowed' : 'pointer', fontSize: '1rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {loadingMore ? <><Loader size={20} className="animate-spin" /> Loading...</> : 'Load More Ornaments'}
                  </button>
                </div>
              )}
            </>
